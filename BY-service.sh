...
binFile="$workDir/BY-Monitor"
pidFile="$workDir/BY-Monitor.pid"
error=""
...
status)
    if [[ -e $pidFile ]]; then
        ...
        pid=`ps aux | grep BY-Monitor | grep -v grep | awk '{print $2}' | head -1`
    fi
    if [[ -z "$pid" ]]; then
        echo "服务不在运行状态"
        exit 1
...
*)
    echo "BY-service启动脚本"
    echo "用法: "
    echo "    ./BY-service.sh (start|stop|restart|status)"
;;
esac
